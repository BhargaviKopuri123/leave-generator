...
if($_SESSION['ROLE']==1){ 
	$sql="select `leave`.*, student.name,student.id as eid from `leave`,student where `leave`.student_id=student.id order by `leave`.id desc";
}else{
	$eid=$_SESSION['USER_ID'];
	$sql="select `leave`.*, student.name ,student.id as eid from `leave`,student where `leave`.student_id='$eid' and `leave`.student_id=student.id order by `leave`.id desc";
}
